...
* COMMENT Current work plan & progress [35%]
...
** TODO Wally as UI tests of separate components
   :LOGBOOK:
   - State "TODO"       from              [2025-08-06 Wed 00:20]
   :END:
*** For testing applicaction in Clojure and Datomic - is there a way to run each test case in a clean database?
...
I've zeroed in on the core challenge: achieving a fresh database state for each test in a Clojure/Datomic context. My initial focus is deconstructing the user's need. I'm thinking about the fundamental problem and the implicit constraints of the request, which seems to revolve around isolated and predictable test environments.


*Refining the Approach*

I'm now zeroing in on a solid strategy. Considering the user's need for a practical solution, I'm leaning toward demonstrating a complete test fixture. My focus is on creating a clear, concise example using =clojure.test/use-fixtures= with Datomic's in-memory database feature. This will include creating, connecting, transacting schema, and deleting the database for each test, directly answering the core problem. I'm aiming for a copy-pasteable code block that showcases the ideal workflow.


*Structuring the Response*

I've crafted the structure of the response to be direct and practical. I'll start with a concise "Yes" to address the immediate question. Subsequently, I'll delve into the "how," explaining test fixtures and showcasing key Datomic and Clojure.test functions. A clear code example, complete with setup and teardown, will be included. I am emphasizing unique database names using =gensym= or UUIDs to mitigate potential parallel test conflicts.


*Finalizing the Code*

I'm deep into the coding phase now. I've chosen a concise and effective code structure, making sure that it directly addresses the user's specific need. The fixture function is solid, using =try...finally= to ensure clean database teardown. My key focus right now is to ensure the example is perfectly copy-pasteable, easy to understand, and covers the essential Datomic and Clojure.test functions. I've added comments to guide the user and made the code self-explanatory.


*Expanding the Solution*

The solution structure is ready, and now I'm thinking about making the example more robust. Beyond the basic setup, I'll explain how to use =:once= fixtures for schema creation to improve test speed, along with the =:each= fixture. I'll cover =d/with= for isolated read access. This expands on the initial response, providing a more versatile, real-world example. It will showcase a few key Datomic features, like transactions and queries. The example will highlight how Datomic's in-memory database significantly simplifies testing, but also touch on potential performance bottlenecks and how to avoid them with the =:once= option. I'll make sure to note that the =:once= approach assumes the schema doesn't change from test to test.


*Expanding the Example*

I've gone beyond the basics and am now adding details to solidify the example. The new version will explain the crucial =:once= and =:each= test fixture approaches and when each is appropriate. It shows how the database can be created, populated, and managed to handle different testing needs. I will include information about isolated test access using =d/with=, so that the user understands a broader range of techniques.
...
*** Please help me define the concept of frontend UI testing in clojure.
 ...
 #+begin_reasoning
 **Exploring Wally Integration*

 I'm currently focused on how to best integrate =wally=, the Playwright wrapper, into frontend UI testing within Clojure. The plan is to create effective tests for the user's interface using Clojure and =wally=. I'm also looking into the best way to structure the testing code. I'm excited by the initial promise of a Clojure and Playwright integration.


 *Assessing Test User Creation*

 I've been thinking about the best way to create test users with different access levels and how that should integrate with =wally=. I'm starting to visualize how I'd use Clojure to streamline user account setup and how I'd structure the testing suite to manage these user scenarios, and their specific access requirements. I'm focusing on creating a workflow that's both efficient and comprehensive.


 *Evaluating Database Setup*

 Now, I'm thinking about how to best seed the database before each test. I'm exploring different approaches, like using fixtures or transactions, and weighing the trade-offs of each method. It feels like a key decision to make early on, to ensure consistent and efficient test execution. I also need to make sure the database setup is easy to manage, since there will be a lot of users and data I'm simulating.


 *Considering E2E Test Structure*

 I'm now thinking about how to structure these UI tests to be as maintainable as possible. I'm considering using the Page Object Model (POM) pattern to separate the UI elements and interactions from the test logic. I'm also exploring different ways to organize tests to improve clarity and readability, especially when dealing with different user roles and their associated scenarios. My goal is to make these tests scalable and resilient as the UI evolves.


 *Developing Test Strategies*

 I've been honing my strategy for testing, specifically focusing on how to set up and tear down the database for each test efficiently, and how to define different test users with specific roles and access. Using fixtures for setup and teardown seems a promising approach, as well as a transaction rollback strategy for database isolation. I'm also considering creating data seeding helpers, to simplify and organize the process of creating test users.


 *Evaluating Test Execution*

 I've been examining how to optimize test execution speed. I'm focusing on using transaction rollbacks to make sure each test starts clean.  I'm also setting up fixtures to streamline test setup and teardown, particularly for things like database connections. The goal is to get a fast and reliable test execution loop using =wally=.


 *Evaluating Test Performance*

 I'm now exploring how to optimize the execution speed of these Clojure E2E tests, particularly in the context of database interaction. While I've implemented transaction rollbacks for isolation, I'm thinking about pre-seeding the database in some cases to speed up the tests. I'm considering using fixtures to further streamline the setup and teardown process to gain maximum speed.
...
Wally is a Clojure Playwright wrapper for browser automation testing. It provides multiple ways to select and interact with web elements.
